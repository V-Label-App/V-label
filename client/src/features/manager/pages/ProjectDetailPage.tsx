...
import type { LegacyLabel as LabelType } from "../../../types/label.types";
...
export function ProjectDetailPage() {
  ...
  const [addImagesMethod, setAddImagesMethod] = useState<
    "later" | "auto" | "manual"
  >("later");
  const [assignmentGroups, setAssignmentGroups] = useState<
    { id: string; annotatorId: string; imageIndices: number[] }[]
  >([]);
  ...
  useEffect(() => {
    if (activeTab === "settings" && project) {
      setTimeout(() => {
        ...
        setEditAnnotators(
          project.assignedTo
            .map((name) => annotators.find((a) => a.name === name)?.id || "")
            .filter(Boolean),
        );
  ...
  const handleAddImagesToProject = () => {
    ...
    if (
      ...
      toast.error(
        `Cannot select more annotators (${addImagesAnnotators.length}) than images (${newImages.length})`,
      );
      return;
    }
    ...
    if (addImagesMethod === "later") {
      ...
      newTasks = newImages.map((file, index) => {
        const group = assignmentGroups.find((g) =>
          g.imageIndices.includes(index),
        );
        const assignedAnnotator = group
          ? annotators.find((a) => a.id === group.annotatorId)
          : null;
  ...
  const handleBatchAssign = () => {
    ...
    const selectedAnnotatorName = annotators.find(
      (a) => a.id === selectedAnnotator,
    )?.name;
    const updatedTasks = project.tasks.map((task) =>
      task.selected
        ? {
            ...
            assignee: selectedAnnotatorName || null,
  ...
  const getAnalyticsData = (project: Project) => {
    const statusData = [
      ...
      {
        name: "Submitted",
        value: project.tasks.filter((t) => t.status === "submitted").length,
        color: "#eab308",
      },
      {
        name: "Approved",
        value: project.tasks.filter((t) => t.status === "approved").length,
        color: "#22c55e",
      },
    ...
    project.tasks.forEach((task) => {
      if (task.assignee)
        annotatorStats[task.assignee] =
          (annotatorStats[task.assignee] || 0) + 1;
    });

    const annotatorData = Object.entries(annotatorStats).map(
      ([name, count]) => ({
        name: name.split(" ")[0],
        tasks: count,
      }),
      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        <Card className="p-6 mb-8">
          <div className="flex items-start justify-between mb-4">
            ...
            <Badge
              variant="outline"
              className={`ml-6 ${isOverdue(project.deadline) ? "border-red-200 bg-red-50 text-red-700" : "border-blue-200 bg-blue-50 text-blue-700"}`}
            >
          ...
          <div className="grid grid-cols-4 gap-4 mt-6">
            ...
            <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-xl font-semibold">
                  {project.tasks.filter((t) => t.status === "assigned").length}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-3 bg-yellow-50 rounded-lg">
              ...
              <div>
                ...
                <p className="text-xl font-semibold">
                  {project.tasks.filter((t) => t.status === "submitted").length}
                </p>
              </div>
            </div>
                              ...
                              <TableCell>
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end">
                                    <DropdownMenuItem
                                      onClick={() =>
                                        handleIndividualAssign(task.id)
                                      }
                                      disabled={task.status !== "pending"}
                                    >
                                      <Users className="w-4 h-4 mr-2" />
      ...
      <Dialog open={isAddImagesOpen} onOpenChange={setIsAddImagesOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload new images and assign to annotators
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 pt-4">
              ...
              {newImages.length === 0 ? (
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      document.getElementById("add-images-upload")?.click()
                    }
                  >
                    Select Images
                  </Button>
            ...
            <div className="space-y-3">
              ...
              <RadioGroup
                value={addImagesMethod}
                onValueChange={(v: "later" | "auto" | "manual") =>
                  setAddImagesMethod(v)
                }
              >
                <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-gray-50">
                  ...
                  <Label htmlFor="add-later" className="flex-1 cursor-pointer">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Add images as pending tasks. Assign manually later.
                    </div>
                  </Label>
                </div>
                ...
                <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-gray-50">
                  ...
                  <Label htmlFor="add-auto" className="flex-1 cursor-pointer">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Each image assigned to exactly one annotator.
                    </div>
                  </Label>
                </div>
                ...
                <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-gray-50">
                  ...
                  <Label htmlFor="add-manual" className="flex-1 cursor-pointer">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Create groups to assign specific images to annotators.
                    </div>
                  </Label>
                </div>
              </RadioGroup>
                  ...
                  <div className="text-sm mt-2">
                    {addImagesAnnotators.length === 0 && (
                      <p className="text-muted-foreground">
                        Select up to {newImages.length} annotators (1 image per
                        annotator)
                      </p>
                    )}
                    {addImagesAnnotators.length > 0 &&
                      addImagesAnnotators.length < newImages.length && (
                        <p className="text-blue-600">
                          ✓ {addImagesAnnotators.length} image
                          {addImagesAnnotators.length !== 1 ? "s" : ""} will be
                          assigned,{" "}
                          {newImages.length - addImagesAnnotators.length} will
                          remain pending
                        </p>
                      )}
                    {addImagesAnnotators.length === newImages.length && (
                      <p className="text-green-600">
                        ✓ Perfect! All {newImages.length} images will be
                        assigned (1 per annotator)
                      </p>
                    )}
                  </div>
                )}
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                ...
                {assignmentGroups.length === 0 ? (
                  <div className="border border-dashed rounded-lg p-6 text-center text-muted-foreground bg-gray-50">
                    ...
                    <p>
                      No assignment groups yet. Create a group to start
                      assigning images manually.
                    </p>
                  </div>
                ) : (
                          ...
                          <div className="grid grid-cols-4 gap-2">
                            {newImages.map((file, imgIndex) => {
                              const isAssignedToThis =
                                group.imageIndices.includes(imgIndex);
                              const isAssignedToOther = assignmentGroups.some(
                                (g, i) =>
                                  i !== groupIndex &&
                                  g.imageIndices.includes(imgIndex),
                              );
                ...
                {newImages.length > 0 && (
                  <div className="text-sm pt-2 border-t">
                    {(() => {
                      const totalAssigned = assignmentGroups.reduce(
                        (acc, g) => acc + g.imageIndices.length,
                        0,
                      );
