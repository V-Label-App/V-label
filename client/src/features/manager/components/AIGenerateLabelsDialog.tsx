...
import { Sparkles, Loader2, Wand2, ArrowLeft, Check, ListChecks } from "lucide-react";
...
export function AIGenerateLabelsDialog({
    ...
    const handleSuggest = async () => {
        ...
        setStep("thinking");
        try {
            const selectedCategory = categoryId === "new"
                ? newCategoryName
...
3. Use your knowledge to provide a high-quality list of labels for this theme.`;
            ...
            try {
                ...
                if (result.type === "card" && result.content?.data?.type === "label_suggestion") {
                ...
                toast.error("AI returned an unexpected response. Please try being more specific.");
                setStep("input");
            }
        } catch (error) {
    ...
    const handleCreate = async () => {
        ...
        setStep("creating");
        try {
            const labelsToCreate = suggestedLabels.filter(label => selectedLabels.has(label));
...
2. This is my final confirmation. Perform the tool call immediately.`;
            ...
            toast.success(`Successfully started creating ${labelsToCreate.length} labels!`);
            onSuccess();
            handleClose();
        } catch (error) {
            console.error("AI creation failed", error);
            toast.error("Failed to create labels. Please try calling the AI in the chat widget directly.");
            setStep("preview");
        }
    };
        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {step === "input" && "Describe the labels you need, and our AI will suggest themes for you."}
                        {step === "thinking" && "AI is analyzing your request and preparing suggestions..."}
                        {step === "preview" && `Review the ${suggestedLabels.length} labels suggested for "${targetCategoryName}".`}
                        {step === "creating" && "Creating your selected labels in the system..."}
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <div className="space-y-4 py-4 animate-in fade-in duration-300">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                <Label htmlFor="description">What labels should I generate?</Label>
                                <Button
                                    variant="ghost"
                                    size="sm"
                                    className="h-7 text-[10px] text-blue-600 hover:text-blue-700 hover:bg-blue-50 gap-1"
                                    onClick={() => setDescription("Tạo 10-15 label về chủ đề [Điền chủ đề ở đây]. Các label nên bao gồm: [Ví dụ 1], [Ví dụ 2]... Hãy đảm bảo các label có tính bao quát và chính xác.")}
                                >
                            ...
                            <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                                ...
                                Tip: You will be able to review and edit the list in the next step.
                            </p>
                        </div>
                    </div>
                )}
                    ...
                    <div className="space-y-4 py-4 animate-in fade-in slide-in-from-right-4 duration-300">
                        <div className="flex items-center justify-between">
                            ...
                            <Button
                                ...
                                onClick={toggleAll}
                            >
                                {selectedLabels.size === suggestedLabels.length ? "Deselect All" : "Select All"}
                            </Button>
                        </div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {suggestedLabels.map((label) => (
                                    <div
                                        ...
                                        className={`flex items-center gap-2 p-2 rounded-md border transition-all cursor-pointer ${selectedLabels.has(label)
