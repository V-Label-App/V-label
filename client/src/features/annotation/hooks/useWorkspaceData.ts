...
export interface UseWorkspaceDataReturn {
    ...
    saveDraft: (annotations: Annotation[], note?: string, timeSeconds?: number) => Promise<void>;
 ...
 */
export const useWorkspaceData = (assignmentId: string): UseWorkspaceDataReturn => {
     ...
     */
    const transformTaskData = useCallback((assignment: TaskAssignmentListItem): WorkspaceTaskData => {
        ...
        const labels = assignment.task.project.projectLabels?.map(pl => ({
            id: pl.label.id,
            name: pl.label.name,
        ...
        return {
            ...
            annotations: [], // Will be populated from assignment.annotations if backend provides it
            labels,
