...
export const projectApi = {
  ...
  getAll: async (params?: {
    ...
    const response = await apiClient.get<ProjectListResponse>(BASE_URL, {
      params,
    });
  ...
  searchPotentialMembers: async (projectId: string, query: string) => {
    const response = await apiClient.get<any[]>(
      `${BASE_URL}/${projectId}/potential-members`,
      {
        params: { search: query },
      },
    );
  ...
  addMember: async (projectId: string, userId: string, role: string) => {
    const response = await apiClient.post(`${BASE_URL}/${projectId}/members`, {
      userId,
      role,
    });
    return response.data;
  ...
  uploadImagesBatch: async (
    ...
    const response = await apiClient.post(
      `${BASE_URL}/${projectId}/images/batch`,
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
  ...
  deleteImage: async (projectId: string, imageId: string) => {
    const response = await apiClient.delete(
      `${BASE_URL}/${projectId}/images/${imageId}`,
    );
    return response.data;
  },
  ...
  getHealthStats: async (projectId: string) => {
    const response = await apiClient.get<ProjectHealthStats>(
      `${BASE_URL}/${projectId}/health`,
    );
    return response.data;
  },
   ...
   */
  getRescueTasks: async (
    projectId: string,
    type: "STUCK" | "PROBLEMATIC" | "ORPHANED",
  ) => {
    const response = await apiClient.get<any[]>(
      `${BASE_URL}/${projectId}/rescue`,
      {
        params: { type },
      },
    );
  ...
  assignTask: async (
    ...
    const response = await apiClient.post(
      `${BASE_URL}/${projectId}/tasks/${taskId}/assign`,
      {
        annotatorId,
  ...
  unassignTask: async (projectId: string, taskId: string) => {
    const response = await apiClient.delete(
      `${BASE_URL}/${projectId}/tasks/${taskId}/unassign`,
    );
    return response.data;
  },
  ...
  bulkAssignTasks: async (
    ...
    const response = await apiClient.post(
      `${BASE_URL}/${projectId}/tasks/bulk-assign`,
      {
        taskIds,
        annotatorId,
  ...
  bulkUnassignTasks: async (projectId: string, taskIds: string[]) => {
    const response = await apiClient.post(
      `${BASE_URL}/${projectId}/tasks/bulk-unassign`,
      {
        taskIds,
      },
    );
   ...
   */
  updateTaskDeadline: async (
    projectId: string,
    taskId: string,
    deadline: Date,
  ) => {
    const response = await apiClient.patch(
      `${BASE_URL}/${projectId}/tasks/${taskId}/deadline`,
      {
        deadline: deadline.toISOString(),
      },
    );
