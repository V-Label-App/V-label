...
'[suggest_labels_for_project] Failed to parse Gemini response:',
 ...
 return ResponseFormatter.asCard({
 title:'No Matching Labels Found',
 variant:'warning',
 fields: {
'Project Description': projectDescription,
 Message:
...
// N. Notify Deadline Tasks (Manager only)
FunctionRegistry.register(
'notify_deadline_tasks',
 async (params, context) => {
 // Security: Only MANAGER or ADMIN
 if (!['MANAGER','ADMIN'].includes(context.userRole)) {
 throw new Error(
'Permission denied: Only MANAGER or ADMIN can send deadline warnings.',
 )
 }

    const { hours_threshold = 24, project_id, confirmed = false } = params
    const threshold = Math.min(Math.max(Number(hours_threshold) || 24, 1), 168)

 // Step 1: Get manager's projects
 ...
 const allProjectIds = managedMembers.map((m) => m.projectId)

 if (allProjectIds.length === 0) {
 return ResponseFormatter.asCard({
 title:'No projects found',
 subtitle:'You are not a manager of any project.',
 icon:'',
 fields: {},
 variant:'default',
 })
 ...
 // Step 2: If no project_id given, show project selection card
 if (!project_id) {
 ...
 const projectButtons: { label: string; action: string; variant: 'primary' | 'secondary'; data: any }[] =
        projects.map((p) => ({
          label: p.name,
          action: 'notify_deadline_tasks',
          variant: 'primary' as const,
          data: { project_id: p.id, hours_threshold: threshold },
        }))

      return ResponseFormatter.asCard({
        title: 'Select a project to check deadlines',
        subtitle: `Will find tasks with deadline within ${threshold} hours`,
        fields: {
          'Projects': projects.length,
          'Time threshold': `${threshold}h`,
        },
    ...
    // Step 3: Verify manager owns the given project
    if (!allProjectIds.includes(project_id)) {
      throw new Error('Permission denied: You are not a manager of this project.')
    }
    const projectIds = [project_id]

 // Step 4: Find task assignments approaching deadline
 const now = new Date()
 const deadlineCutoff = new Date(now.getTime() + threshold * 60 * 60 * 1000)
 ...
 orderBy: { deadline:'asc'},
    ...
    if (assignments.length === 0) {
      return ResponseFormatter.asCard({
        ...
        subtitle: `No tasks with deadline within ${threshold} hours.`,
        fields: { 'Threshold': `${threshold}h`, 'Projects checked': projectIds.length },
        variant: 'success',
      })
    }

    // Group by annotator
    const byAnnotator = new Map<string, typeof assignments>()
    for (const a of assignments) {
      const list = byAnnotator.get(a.annotatorId) ?? []
      list.push(a)
      byAnnotator.set(a.annotatorId, list)
    }

    // Step 5: Show confirmation card if not yet confirmed
    if (!confirmed) {
      const previewFields: Record<string, string> = {}
      for (const [, tasks] of byAnnotator) {
        const name = tasks[0]!.annotator.fullName || tasks[0]!.annotator.email
        previewFields[name] = `${tasks.length} task${tasks.length > 1 ? 's' : ''} approaching deadline`
      }

      return ResponseFormatter.asCard({
        title: 'Send deadline warnings?',
        subtitle: `${byAnnotator.size} annotator(s) will be notified`,
        ...
        actions: [
          {
            ...
            action: 'notify_deadline_tasks',
            variant: 'primary',
            data: { project_id, hours_threshold: threshold, confirmed: true },
          },
          {
            label: 'Cancel',
            action: 'Cancel',
    ...
    for (const [annotatorId, tasks] of byAnnotator) {
      const annotatorName = tasks[0]!.annotator.fullName || tasks[0]!.annotator.email
      const projectNames = [...new Set(tasks.map((t) => t.task.project.name))]

      const notifTitle =
        tasks.length === 1
          ? `You have 1 task approaching deadline`
          : `You have ${tasks.length} tasks approaching deadline`

      const notifMessage = `in project: ${projectNames.join(', ')}`
      ...
      const notification = await NotificationService.createNotification({
        ...
        metadata: {
          ...
          projectIds: [...new Set(tasks.map((t) => t.task.projectId))],
          nearestDeadline: tasks[0]!.deadline!.toISOString(),
        },
      })
      ...
      // Send email via DEADLINE_WARNING template
      const taskListText = tasks.map((t) => {
        const hoursLeft = Math.round((t.deadline!.getTime() - now.getTime()) / (1000 * 60 * 60))
        return `• ${t.task.project.name} — due in ${hoursLeft} hour${hoursLeft !== 1 ? 's' : ''}`
      }).join('\n')

      const emailService = new EmailService()
      await emailService.sendEmail({
        to: tasks[0]!.annotator.email,
        templateType: 'DEADLINE_WARNING',
        variables: {
      ...
      notifiedCount++
      summaryFields[annotatorName] = `${tasks.length} task${tasks.length > 1 ? 's' : ''} approaching deadline`
    }

    return ResponseFormatter.asCard({
      title: `Deadline warning sent to ${notifiedCount} annotator(s)`,
      subtitle: 'In-app notification and email sent',
      fields: summaryFields,
      variant: 'warning',
    })
 },
 {
 description:`Find tasks approaching deadline in the manager's projects and send DEADLINE_WARNING notifications to assigned annotators.

IMPORTANT: If project_id is not provided, call this function immediately — it will automatically display a project selection card for the user to choose from. Do NOT ask the user to select a project first.

Examples:
- "Any tasks near deadline?" → call immediately, no project_id needed
- "Remind annotators about deadlines" → call immediately
- "Tasks due in 48 hours?" → hours_threshold=48
- After user selects a project → project_id is passed in automatically`,
 parameters: {
 type:'object',
 properties: {
 hours_threshold: {
 type:'number',
 description:'Warn tasks with deadline within X hours (default: 24, max: 168)',
 },
        project_id: {
          type: 'string',
          description: 'UUID of the specific project to check. Omit if not yet selected — a selection card will be shown.',
        },
        confirmed: {
          type: 'boolean',
          description: 'Set to true only after user clicks "Send notifications" on the confirmation card. Never set this manually.',
        },
      },
    },
  },
