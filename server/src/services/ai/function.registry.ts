...
FunctionRegistry.register('send_system_announcement', async (params, context) => {
  // Security: Only ADMIN can send system announcements
  if (context.userRole !== 'ADMIN') {
    throw new Error('Permission denied: Only ADMIN can send system announcements.');
  }
  ...
  const roleKeywords = ['manager', 'admin', 'annotator', 'reviewer', 'quản lý', 'người đánh giá'];
  if (title && roleKeywords.some(r => title.toLowerCase().includes(r)) && !message) {
  ...
  if (targetType === 'roles' && targetRoles) {
    options.targetRoles = targetRoles.split(',').map((r: string) => r.trim().toUpperCase()).filter(Boolean);
  } else if (targetType === 'emails' && targetEmails) {
    options.targetEmails = targetEmails.split(',').map((e: string) => e.trim().toLowerCase()).filter(Boolean);
  }
  ...
  return ResponseFormatter.asCard({
    ...
    actions: [
      { label: 'Gửi thông báo khác', action: 'Gửi thông báo hệ thống', variant: 'primary' }
    ]
  });
}, {
...
Only show form for title/message if not provided. DO NOT ask for targetType/targetRoles if already clear from context.`,
  parameters: {
    type: 'object',
    properties: {
      title: { type: 'string', description: 'Announcement title (show form if missing)' },
      message: { type: 'string', description: 'Announcement message content (show form if missing)' },
      targetType: { type: 'string', description: 'Auto-detect from user message: "all", "roles", or "emails"', enum: ['all', 'roles', 'emails'] },
      targetRoles: { type: 'string', description: 'Auto-extract roles mentioned: ADMIN, MANAGER, REVIEWER, ANNOTATOR' },
      targetEmails: { type: 'string', description: 'Auto-extract emails mentioned in user message' }
    }
  }
});

// 6. Create Labels Automatically (Manager/Admin only)
FunctionRegistry.register('create_labels_auto', async (params, context) => {
  // Security: Only MANAGER or ADMIN can create labels
  if (context.userRole !== 'MANAGER' && context.userRole !== 'ADMIN') {
    throw new Error('Permission denied: Only MANAGER or ADMIN can create labels.');
  }

  const { categoryName, categoryDescription, labels, useExistingCategory } = params;
  ...
  try {
    ...
    if (categoryName) {
      ...
      if (!matchedCategory) {
        ...
        for (const [vietnameseKey, englishKeywords] of Object.entries(categoryKeywordMap)) {
          if (lowerCategoryName.includes(vietnameseKey) ||
            englishKeywords.some(kw => lowerCategoryName.includes(kw))) {
            // Find existing category that matches these keywords
            matchedCategory = existingCategories.find(c =>
              englishKeywords.some(kw => c.name.toLowerCase().includes(kw)) ||
      ...
      if (matchedCategory) {
        if (useExistingCategory === false) {
          ...
          results.errors.push(`Category "${matchedCategory.name}" already exists. Set useExistingCategory=true to add labels to it.`);
          return ResponseFormatter.asCard({
            ...
            fields: {
              ...
              'Suggestion': 'Use the existing category or choose a different name'
            },
            actions: [
              { label: 'Use Existing Category', action: `Thêm labels vào category ${matchedCategory.name}`, variant: 'primary' },
              { label: 'Choose Different Name', action: 'Tạo category mới với tên khác', variant: 'secondary' }
            ]
          });
        }
        ...
        const newCategory = await LabelCategoryService.create({
          name: categoryName,
          description: categoryDescription || `Auto-generated category for ${categoryName}`
        });
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      const labelName = typeof labelData === 'string' ? labelData : labelData.name;
      let labelColor = typeof labelData === 'object' && labelData.color ? labelData.color : null;
      ...
      if (existingLabelNames.includes(labelName.toLowerCase())) {
        results.labelsSkipped.push({ name: labelName, reason: 'Already exists' });
        continue;
      }
      ...
      try {
        ...
        if (err.code === 'P2002') {
          results.labelsSkipped.push({ name: labelName, reason: 'Duplicate name' });
        } else {
          results.errors.push(`Failed to create "${labelName}": ${err.message}`);
        }
      }
    }
    ...
    if (results.labelsCreated.length === 0 && results.errors.length === 0) {
      return ResponseFormatter.asCard({
        ...
        fields: {
          ...
          'Skipped': results.labelsSkipped.map(s => `${s.name} (${s.reason})`).join(', ') || 'None'
        }
      });
    }
    ...
    summaryFields['Labels Created'] = results.labelsCreated.length > 0
      ? `${results.labelsCreated.length} labels`
      : 'None';

    if (results.labelsCreated.length > 0) {
      summaryFields['Label Names'] = results.labelsCreated.length <= 10
        ? results.labelsCreated.join(', ')
        : results.labelsCreated.slice(0, 10).join(', ') + ` (+${results.labelsCreated.length - 10} more)`;
    }

    if (results.labelsSkipped.length > 0) {
      summaryFields['Skipped'] = results.labelsSkipped.length <= 5
    ...
    if (results.labelsCreated.length > 0) {
      ...
      console.log('[Function] Broadcasted LABEL_CREATED event:', results.labelsCreated.length, 'labels');
    }
    ...
    return ResponseFormatter.asCard({
      ...
      actions: [
        { label: 'Tạo thêm labels', action: 'Tạo thêm labels mới', variant: 'primary' },
        { label: 'Xem tất cả labels', action: 'Xem danh sách labels', variant: 'secondary' }
      ]
    });
...
- Match Vietnamese category names to English equivalents: "xe cộ" → "Vehicles", "động vật" → "Animals"`,
  parameters: {
    ...
    properties: {
      categoryName: {
        type: 'string',
        description: 'Name of category to create or use. Extract from user message or generate based on theme.'
      },
      categoryDescription: {
        type: 'string',
      ...
      labels: {
        ...
        description: 'Array of label names to create. AI MUST generate this based on user description (e.g., "10 animals" → generate 10 animal names)'
      },
      useExistingCategory: {
        type: 'boolean',
        description: 'If true, add labels to existing category with same name. If false/undefined and category exists, show warning.'
      }
    },
    required: ['labels']
  }
...
FunctionRegistry.register('get_label_categories', async (params, context) => {
  // Available for Manager, Admin
  if (!['MANAGER', 'ADMIN'].includes(context.userRole)) {
    throw new Error('Permission denied: Only MANAGER or ADMIN can view label categories.');
  }
  ...
  try {
    ...
    if (categories.length === 0) {
      return {
        categories: [],
        message: 'No categories found. You can create one using create_labels_auto function.'
      };
    }
...
FunctionRegistry.register('get_all_labels', async (params, context) => {
  // Available for Manager, Admin (and maybe Reviewer/Annotator for read)
  if (!['MANAGER', 'ADMIN', 'REVIEWER', 'ANNOTATOR'].includes(context.userRole)) {
