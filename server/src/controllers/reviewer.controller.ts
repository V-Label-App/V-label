...
export class ReviewerController {
  ...
  static async getAssignmentDetail(req: Request, res: Response) {
    try {
      ...
      const result = await ReviewerService.getAssignmentDetail(
        assignmentId,
        userId,
      )
      return res.json(result)
      ...
      return res
        .status(500)
        .json({ error: 'Failed to fetch assignment detail' })
    }
  }
  ...
  static async rejectTask(req: Request, res: Response) {
    try {
      ...
      if (!reviewComment || reviewComment.trim() === '') {
        return res
          .status(400)
          .json({ error: 'Review comment is required when rejecting a task' })
      }

      const result = await ReviewerService.rejectTask(
        assignmentId,
        userId,
        reviewComment,
      )
